...
// 處理會議資訊頁
module.exports = async (confId, user, users, confRooms,confRooms2) => {

    // 從會議主檔中取得這筆會議的資料
    const confInfo = await Conf.findOne({ _id: confId }).lean()
    //取得room2
    let room2namedata=await SecondRoom.findOne({ confId: confId }).lean()
    // 從會議記錄檔中取得會議檔案
    ...
    // 從空間2檔中取得空間資料
    const room2info=await SecondRoom.findOne({ confId: confId }).lean()
    
    return { confInfo, roomInfo, attendeeList, minutes, organizer, chairSign, roomSign, identity,room2info }
